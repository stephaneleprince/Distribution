...
var FU = {
  ...
  add: function(file) {
    ...
    if (currentDirectoryId == 0) {
      modal.simpleContainer(null, Translator.trans('desktop_root_directory_upload_error', {}, 'platform'))
      return
    }
...
function onFiles(files) {
  ...
  FileAPI.each(files, function(file) {
    if (maxSize !== 0) {
      switch (lastChar) {
        case 'M':
          size = varSize * FileAPI.MB
          break
        case 'K':
          size = varSize * FileAPI.KB
          break
        case 'G':
          size = varSize * FileAPI.GB
          break
      }
    }
